...
namespace Tproject.CutScene
{
    ...
    public class CutSceneController : MonoBehaviour
    {
        ...
        public Dictionary<string, ContentSection> contentSectionsDict;
        public AudioClip backsoundCutScene;

        public List<Sprite> playerSpeakers;
        ...
        public void SetUpPlayerSpeaker(int charIndex)
        {
            for (int i = 0; i < contentSections.Count; i++)
            {
                if (contentSections[i].title == "Charlotte Wilson,LLB")
                {
                    ContentSection newData = contentSections[i];
                    newData.speakerImage = playerSpeakers[charIndex];
                    contentSections[i] = newData;
                    // Debug.Log(i);
                }
            }
        }
        ...
        private void RequestTypeText()
        {
            ...
            if (scriptReady.background != null)
            {
                ...
                _myCoroutine = StartCoroutine(TypeText(scriptReady.title, scriptReady.contentText, scriptReady.speakerImage, scriptReady.sfx, scriptReady.background));
            ...
            else
            {
                ...
                LeanTween.scale(GO_boxPanel, new Vector3(1f, 1f, 1f), 1f).setDelay(.5f).setEase(LeanTweenType.easeOutElastic);
                _myCoroutine = StartCoroutine(TypeText(scriptReady.title, scriptReady.contentText, scriptReady.speakerImage, scriptReady.sfx));
            }


        }

        private IEnumerator TypeText(string title, string contentScript, Sprite speakerImg, AudioClip clip = null, Sprite bg = null)
        {
            ...
            UI_speakerImg.sprite = speakerImg;
